...
var textarea = load('textarea'); textarea.onTextareaChange({ detail: { value: '多行内容' } }); assert.strictEqual(textarea.data.textareaValue, '多行内容'); textarea.onTextareaClear(); assert.strictEqual(textarea.data.textareaValue, '');
var search = load('search'); search.onSearchChange({ detail: { value: 'tabs' } }); assert.strictEqual(search.data.searchValue, 'tabs'); search.onSearchConfirm({ detail: { value: 'tabs' } }); assert.ok(search.data.searchStatus.indexOf('tabs') !== -1); search.onSearchCancel(); assert.strictEqual(search.data.searchValue, '');
...
var select = load('select'); select.onSelectChange({ detail: { value: 'preview' } }); assert.strictEqual(select.data.selectValue, 'preview');
var picker = load('picker'); picker.onOpenPicker(); assert.strictEqual(picker.data.pickerVisible, true); picker.onPickerChange({ detail: { value: ['preview'] } }); assert.deepStrictEqual(picker.data.pickerValue, ['preview']); picker.onPickerVisibleChange({ detail: { visible: false } }); assert.strictEqual(picker.data.pickerVisible, false);
