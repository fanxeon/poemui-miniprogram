...
assert(visible.instance.data.rootClass.includes('pui-notice-bar--danger'));
...
assert((wxml.match(/icon-only/g) || []).length >= 2, 'NoticeBar prefix/suffix icon actions must use fixed icon-button geometry');
const suffixButton = wxml.slice(wxml.indexOf('custom-class="pui-notice-bar__suffix-button"'));
assert(suffixButton.includes('variant="transparent"'), 'NoticeBar suffix action must use the transparent Button variant');
assert(!suffixButton.includes('theme="default"'), 'NoticeBar suffix action must not declare the default Button theme');
assert(!suffixButton.includes('variant="base"'), 'NoticeBar suffix action must not create a second muted Surface');
assert(!suffixButton.includes('surface="transparent"'), 'NoticeBar suffix action keeps circle geometry instead of removing the Button radius');
...
assert(previewStyles.includes('.pui-notice-preview__icon--suffix { margin-left: auto; }'));
assert(wxss.includes('.pui-notice-bar--primary') && wxss.includes('.pui-notice-bar__icon--suffix'));
assert(wxss.includes('.pui-notice-bar__suffix-button') && wxss.includes('color: inherit !important;'), 'transparent suffix action inherits the NoticeBar semantic foreground');
