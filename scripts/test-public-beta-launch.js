...
const publicRegistryVersion = '0.1.4';
...
assert.strictEqual(packageJson.version, '0.1.4', '本地工作树必须先升级到 0.1.4');
assert(previewApp.includes(`npm i poemui-miniprogram@${publicRegistryVersion} -S --production`), '快速开始必须提供当前发布目标的 Registry 固定版本');
...
assert(previewApp.includes("['getting-started', 'release-notes'].includes(detail.id) ? '' : propWorkspaceMarkup(detail)"), '快速开始与更新公告不得暴露 done、内部路径或文档调试面板');
...
assert(readme.includes(`当前公开版本为 \`poemui-miniprogram@${publicRegistryVersion}\``), 'README 必须显式声明公共发布版本');
assert(readme.includes(`当前公开版本为 \`poemui-miniprogram@${publicRegistryVersion}\``), 'README 必须声明当前公开发布版本');
