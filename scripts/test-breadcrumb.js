...
const wxss = read('breadcrumb/breadcrumb.wxss');
...
assert(js.includes("wrap: { type: Boolean, value: false }"), 'Breadcrumb defaults to a complete single-line path with local horizontal scrolling');
assert(preview.includes("size: 'medium', wrap: false"), 'H5 Breadcrumb shares the nowrap default');
assert(wxml.includes('class="pui-breadcrumb__state-message"'), 'Breadcrumb error and empty copy keep a bounded message track beside the action');
assert(/\.pui-breadcrumb__state--error\s*\{[^}]*display:\s*grid/s.test(wxss), 'native error state must activate its three-column grid');
assert(wxss.includes('grid-template-columns: auto minmax(0, 1fr) auto'), 'native error state keeps message and retry on separate bounded tracks');
