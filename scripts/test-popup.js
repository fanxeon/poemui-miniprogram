...
assert(wxml.includes("pui-popup__content--with-header"), 'Popup Content must expose the Header adjacency state instead of relying on a duplicated Surface gap');
...
assert(/\.pui-popup__surface\s*\{[^}]*gap:\s*0;/.test(wxss), 'Popup Surface must not add a second Header-to-Content section gap');
assert(/\.pui-popup__content--with-header\s*\{[^}]*padding-top:\s*var\(--pui-popup-content-gap\);/.test(wxss), 'Popup Header-to-Content spacing must use the single compact content gap');
assert(/\.pui-spacing--equal \.pui-popup__content--with-header\s*\{[^}]*padding-top:\s*var\(--pui-popup-panel-padding\);/.test(wxss), 'equal spacing must remap the single Header-to-Content gap instead of restoring a Surface gap');
...
assert(preview.includes('pui-popup-preview__scroll--with-header'), 'H5 Popup must mirror the native Header adjacency class');
...
assert(/\.pui-popup-preview__surface\s*\{[^}]*gap:\s*0;/.test(previewStyles), 'H5 Popup Surface must not reintroduce the removed section gap');
assert(/\.pui-popup-preview__scroll--with-header\s*\{[^}]*padding-top:\s*var\(--pui-preview-content-gap\);/.test(previewStyles), 'H5 Header-to-Content spacing must use the same compact content gap');
...
assert(popupApi.includes('<pui-popup visible="{{true}}" content="Popup 内容" />'));
assert(popupApi.includes('Starter Usage'));
