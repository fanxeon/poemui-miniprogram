...
assert(/\.pui-input__clear\s*\{[^}]*margin-left:\s*auto;/.test(wxss), 'clear Button must occupy the Field trailing edge when no suffix follows');
...
assert(/\.pui-input-preview__control > \.pui-input-preview__clear\s*\{[^}]*margin-left:\s*auto;/.test(previewStyles), 'H5 clear Button mirrors the trailing edge placement');
