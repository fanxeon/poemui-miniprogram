...
assert(wxss.includes('.pui-combobox__option { color: var(--pui-text-primary); border-radius: var(--pui-radius-medium);'), '小程序独立选中 Option 必须消费 medium 语义圆角，而不是由页面样式补偿');
...
assert(previewStyles.includes('background: transparent; border: 0; border-radius: var(--pui-site-radius-control); cursor: pointer;'), 'H5 独立选中 Option 必须跟随站点控制圆角，不能在果味外观中保持直角');
