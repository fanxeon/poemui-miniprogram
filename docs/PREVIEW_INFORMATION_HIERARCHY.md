...
目录以 `taxonomyId` 为唯一归属依据，不能再根据源码目录或 H5 路由名临时分组。一个规范公开入口只能出现一次，不创建第二套“适配组件”分区。0.1.4 工作树有 78 个可安装的 npm 组件，H5 另含快速开始与设计规范文档入口。小程序 Search 不包含 H5 快速开始入口，因此是 78 个组件加 5 个规范页，共 83 条候选。旧 Chart 研究入口已由 AreaChart、BarChart、Waffle、DonutChart、RadarChart 五个真实组件替代。
...
- 常规模式左侧常驻透明、可滚动的组件用法正文，按“组件引用”小标题与 `page.json usingComponents` 代码块、“基础用法”小标题与 Starter WXML 代码块自然展示；数据驱动或命令式组件随后追加必要的“页面数据/页面逻辑”。各段不使用 Tabs，也不形成独立滚动卡片。总 Header 和常驻默认值说明删除，正文根不建立 Surface、边界、圆角、阴影或 padding，并承担唯一纵向滚动。标题行分别通过共享 CodeBlock helper 组合 PUI Copy IconButton，复制当前段代码，并以 Check/ErrorCircle、可访问名称和 `aria-live` 反馈真实结果。可见引用文档读取 `metadata/component-starter-usage.js`，与运行时安全默认值及 H5 Showcase 初态分离；工具栏复制仍通过 `makeUsageCode` 输出当前实时效果。代码块保留 code Surface、可读等宽字体、行号、层级缩进、语法颜色、键盘焦点与必要横向滚动。基础开始标签以 `80` 字符软上限、每行最多 3 个属性合并。桌面把 PreviewDevice 推到 Stage 右侧 gutter，正文从左侧 gutter 撑满至设备前并保留 14px 间距，两者同为 622px 高；窄于 `1180px` 时正文落到左下满宽 240px 有界区域。切换选择元素后设备恢复居中，由双侧 Inspector 接管该辅助层。模式切换保留同一个设备 DOM 和组件运行态，低动效压缩为 1ms。
