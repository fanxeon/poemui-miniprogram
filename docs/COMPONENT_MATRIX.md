...
当前 npm 包包含 69 个组件目录：`done` 69 个，`beta` 0 个，`experimental` 0 个。
...
| 组件/能力 | 状态 | 路径 | 当前交付边界 |
| --- | --- | --- | --- |
| Getting Started 安装 | done | `npm i poemui-miniprogram` | 安装、构建 npm、按需引用、主题配置 |
...
| Date Picker | done | `poemui-miniprogram/date-time-picker/date-time-picker` | `value`、`defaultValue`、`visible`、`defaultVisible`、`mode`、`start`、`end`、`format`、`steps`、`showWeek`、`title`、`cancelText`、`confirmText`、`showHeader`、`usePopup`、`autoClose`、`closeOnOverlayClick`、`disabled`、`readonly`、`ariaLabel`、`reduceMotion` |
...
| Popup 弹出层 | done | `poemui-miniprogram/popup/popup` | `closeBtn`、`showHeader`、`title`、`subtitle`、`showFooter`、`closeOnOverlayClick`、`content`、`card`、`duration`、`overlayProps`、`placement`、`preventScrollThrough`、`showOverlay`、`blurOverlay`、`usingCustomNavbar`、`visible`、`defaultVisible`、`zIndex`、`ariaLabel`、`reduceMotion` |
