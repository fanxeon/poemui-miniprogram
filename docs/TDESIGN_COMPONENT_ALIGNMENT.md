...
- 2026-07-29 联网固定参考 TDesign `1.15.3` 官方 Avatar 文档、GitHub `packages/components/avatar` 与安装包 `miniprogram_dist/avatar/{props.js,type.d.ts,avatar.js,avatar.wxml,avatar.wxss}`。TDesign 安装源码为 9 Props、1 error Event、默认 Slot，AvatarGroup 另有独立合同；PoemUI 最终保留 13 Props、1 error Event、1 default Slot、0 业务 Methods，不新增原本不存在的 AvatarGroup 包。
- Avatar 明确收敛为展示叶子：删除 `image` 别名、`clickable/disabled`、自身 `click/load` 和私有 `duration/easing`；保留 `src/text/alt/icon/useSlot` 回退、聚焦的 `lazy`、内部 Loading 强制态、三档尺寸/形状、bordered、失败隐藏、ARIA 与 reduceMotion。非空 src 的资源等待和 `loading=true` 持续态均由 Avatar 内部 PUI Loading 绘制；图片 load 只更新内部 opacity，失败才向消费者发布 error。宿主业务的 disabled/selected 与业务请求 loading 仍由外层 PUI Button/Cell 承担。
- 基础 WXML 为 `<pui-avatar text="P" />` 且零 `bind:*`。概览按“基础用法 / 懒加载、图片与回退 / 尺寸与形状 / 组合用法”分区，组合区使用真实 Avatar + Badge + PUI Button；点击后按钮自身回写“资料已打开”，禁用宿主保持 disabled，Avatar 内部始终没有 button。
- 浏览器 battle 必须验证内部 Loading → load/error、lazy/eager、缓存完成、src 快速切换、失败回退、`hideOnLoadFailed`、500ms 中间/完成态、reduceMotion=1ms、`text=0`、空字符串、Icon/Slot、尺寸/形状/边线、宿主门禁、元素选择、重置和 API 12/1/1。H5 三档尺寸固定镜像原生 28/40/56px，展示叶子保持无外投影。
...
- 固定参考 TDesign `1.15.3` 官方 CountDown 文档与安装包 `miniprogram_dist/count-down/props.js`、`type.d.ts`、WXML、WXSS 和 JS。TDesign 公开 8 Props、2 Events 与 `start/pause/reset`；PoemUI 0.1.1 收敛为 11 Props，0.1.2 按下方数字滚动扩展为 12 Props，Events、Slot 与 Methods 不变。
- 当前 Props 为 `time/autoStart/paused/content/format/millisecond/size/theme/splitWithUnit/animation/ariaLabel/reduceMotion`。删除 `finishText/customContent/pauseOnHidden/duration/easing`，删除重复的 `start/pause/reset` Events、`restart/getRemaining` 和 prefix/suffix Slots；保留 PoemUI 的声明式 paused、ARIA、统一低动效与真实 `getTime()`。
...
- 基础 WXML 为 `<pui-count-down />`，零 `bind:*`。官网 0.1.1 重建为“基础用法 / 主题与尺寸 / 单位与毫秒 / 控制与自定义内容”四区，Start/Pause/Reset/读取剩余均调用真实 PUI Button 和同一计时状态机；0.1.2 H5 尚待统一增加动效风格分区并把 API 更新为 12/2/1/4。
...
### CountDown · 2026-07-29 数字滚动扩展

- 当日重新访问 [TDesign CountDown 文档](https://tdesign.tencent.com/miniprogram/components/count-down) 与 [官方源码目录](https://github.com/Tencent/tdesign-miniprogram/tree/develop/packages/components/count-down)，并重新解包固定 `tdesign-miniprogram@1.15.3`，读取 `miniprogram_dist/count-down/{props.js,type.d.ts,count-down.js,count-down.wxml,count-down.wxs,count-down.wxss}`。官网仍只提供基础/尺寸示例与 8 项核心 Props；npm 网页返回 403，registry 版本与固定安装包仍可验证。
- TDesign 没有数字逐位滚动 API。PoemUI 因用户真实展示需求新增单一 `animation="pulse|roll"`，默认 `pulse` 保持兼容；不恢复 `duration/easing`，也不增加 flip、完成事件或第二套计时状态。
- `roll` 只给发生变化的 `DD/HH/mm/ss` 数字位保留旧值/新值并垂直滚动，未变化位静止；`SSS` 直接刷新，避免 50ms 更新持续重启 500ms 动效。切换风格不重置 `targetTime`，`change/finish` 与四个 Methods 不变。
- H5 的 `preview/app.js`、`preview/styles.css`、metadata helper/WXML 与 390px 验收留待 0.1.2 最终批次统一同步；本次只交付小程序共享源码、真实独立页、合同、Ledger 与专项测试。
...
- 固定参考 TDesign `1.15.3` 官方 Input 文档、GitHub `packages/components/input` 与安装包 `props.js/type.d.ts/WXML/WXSS/JS`。PoemUI 不按数量追平，当前为 31 Props、5 Events、7 Slots、4 Methods；新增差异仅为 `clearTrigger=focus/always`，普通 Input 默认 focus，Search 显式保留自身 always/focus 策略。
...
- 固定参考 TDesign `1.15.3` 官方 Textarea 文档，以及安装包 `miniprogram_dist/textarea/props.js`、`type.d.ts`、WXML 和 JS。TDesign 当前为 28 Props、6 Events、1 label Slot、0 Methods；PoemUI 不按数量追平，0.1.2 当前收敛为 29 Props、6 Events、3 Slots、3 Methods。
- 当前 Props 为 ``；Events 为 `change/focus/blur/enter/line-change/keyboardheightchange`；Slots 为 `label/tips/extra`；Methods 为 `focus/blur/getValue`。删除旧版重复自动高度、计数、错误、Slot开关、Clear 与调试方法；maxcharacter 优先于 maxlength，退控保留最后受控值。
- 基础 WXML 为 `<pui-textarea />`，零 `bind:*`。官网固定为“基础用法 / 字符计数 / 自动增高 / 状态与交互”四区；当前实例、固定示例、属性页、复制代码和兼容说明共用同一29/6/3/3合同，H5不伪造 line-change 或 keyboardheightchange。
- 2026-07-19 的 30/7/3/4 浏览器 battle 曾实测全部旧 Props、Clear 与方法；这部分只保留为历史证据，不再代表 0.1.2 当前公开 API。仍有效的证据包括 `0/false/空字符串` 受控值、maxcharacter 截断、autosize、focus、低动效、390px、主题和外观。
...
- 2026-07-28 组件页回归再次联网读取 [TDesign 官方 Textarea 页面](https://tdesign.tencent.com/miniprogram/components/textarea)、[官方仓库目录](https://github.com/Tencent/tdesign-miniprogram/tree/develop/packages/components/textarea)，并固定解包 `tdesign-miniprogram@1.15.3` 的 `props.js/type.d.ts/JS/WXML/WXSS`。PoemUI 发现自己的受控 `onInput` 会返回 `previousValue`，导致微信原生 Textarea 先回退旧文本、父级随后回写新文本，`auto-height` 在一次输入内重复测量。现改为正常受控输入返回 `undefined`，只有真实截断才返回规整值；专项锁定输入回调自身新增 `setData=0`、父级值回写后子组件同步 `setData=1`。Ledger 更新为 `PUI-FB-0472`。
- 用户复看后仍观察到输入高度先增后减。PoemUI 0.1.2 按 TDesign 无 Clear 的合同删除 `clearable` Prop、`clear` Event、`clear()` Method、内部 PUI Button 和 H5 入口，但用户再次确认闪动仍在，说明 Clear 不是主因。临时 line-change 取证只有初始化 `1行/21px` 与输入后 `6行/126px` 的单调事件；最终修复改为逻辑 `innerValue` 与原生 `renderValue` 分离，并差量提交派生状态，匹配当前草稿的父级 value echo 为零组件 `setData`，真正不同的外部 value 才重绑原生 textarea。当前合同仍为 29 Props / 6 Events / 3 Slots / 3 Methods。
...
- 2026-07-29 再次访问 [TDesign Picker 官方页](https://tdesign.tencent.com/miniprogram/components/picker) 与 [官方 Picker 源码](https://github.com/Tencent/tdesign-miniprogram/tree/develop/packages/components/picker)，并重新解包固定 `tdesign-miniprogram@1.15.3` 的 `miniprogram_dist/picker/{props.js,type.d.ts,picker.js,picker.wxml,template.wxml,picker.wxss}`。固定源码的 `116rpx` Toolbar 后直接进入 Main，没有再叠 section gap，默认 `visibleItemCount=5`。PoemUI 因此把 Select/Picker 的 Header→选择内容收口为一个 `--pui-surface-inset`：普通模式由 Popup Content padding 提供，equalSpacing 由 section gap 提供；Select 同时删除 Options 自身重复 padding，Picker 保留 5 行、itemHeight 与中心选中。当前官方仓库没有独立 Select 组件目录，PoemUI Select 继续使用 Button + Popup 的简单单选合同。
...
- 2026-07-28 因用户复核取消仍过大，再次访问 [TDesign Search 官方页](https://tdesign.tencent.com/miniprogram/components/search) 与 [官方 Search 源码](https://github.com/Tencent/tdesign-miniprogram/tree/develop/packages/components/search)，并读取固定 `tdesign-miniprogram@1.15.3` 的 `miniprogram_dist/search/{props.js,type.d.ts,search.js,search.wxml,search.wxss}`。固定源码把右侧 action 作为 `32rpx/48rpx` 的独立 view；PoemUI 不照搬大号 action，也不退回原生点击根，而是继续组合 PUI Button。审计确认此前 transparent/extra-small 仍继承 `88rpx` 最小宽度；改用 Button 正式 `text` 合同后，微信自定义组件宿主仍实际拉至 `184px`，且 `auto/fit-content/min-content` 无法收缩。最终由 Search 按取消文案字符权重选择 `68/104/144/176rpx` 四级宿主宽度，默认取消为 `34px`，同时保留 extra-small 高度、secondary tone、medium 字重和 transparent Surface；H5 同步留待 0.1.2 最终汇总，Ledger 更新 `PUI-FB-0473`。
...
- 2026-07-28 回归复核重新访问 [TDesign Steps 官方页](https://tdesign.tencent.com/miniprogram/components/steps) 与 [npm 1.15.3 发布页](https://www.npmjs.com/package/tdesign-miniprogram)，并读取固定包 `miniprogram_dist/steps/steps.wxss`、`step-item/step-item.{wxml,wxss}`。固定源码的纵向线以 `circle-size / 2` 为 x 锚点、从 `circle-size + 16rpx` 开始；PoemUI 审计发现自身旧规则误把 `44rpx` 完整尺寸写入 left，旧测试也因此假通过。现以组件内部几何别名同步小程序/H5 的 default/dot、horizontal/vertical 四种组合，并以数值关系和源码公式双重锁定；Ledger 更新 `PUI-FB-0427`。
...
- 2026-07-29 因用户指出头像轮廓不是正圆，再次访问官方页与官方组件目录，并重新解包 `tdesign-miniprogram@1.15.3` 的 `miniprogram_dist/skeleton/{props.js,type.d.ts,skeleton.js,skeleton.wxml,skeleton.wxss}`。固定源码为 circle 只设默认 width/height、不固定更大的 min-height，并以 `var(..., 50%)` 最终兜底；PoemUI 因旧 `min-height:96rpx` 把页面 `size=72rpx` 拉成椭圆，现改为 `min-height:0 + border-radius:50%`。公开面继续是 7 Props、1 默认 Slot、0 Events、0 Methods，不恢复 avatar/avatarSize/avatarShape。
...
- 2026-07-29 因用户指出内部 default Close 主次过强，再次访问上述官方页面与仓库，并重新解包固定 `tdesign-miniprogram@1.15.3` 的 `miniprogram_dist/notice-bar/{props.js,type.d.ts,notice-bar.js,notice-bar.wxml,notice-bar.wxss}`。固定源码把 suffix 作为 `44rpx` 图标区，并通过 `--td-notice-bar-suffix-icon-color` 控制前景，没有第二层 Button Surface。PoemUI 仍按全库组合合同使用 PUI Button，但改为 `variant=transparent + small + circle + icon-only`：保留命中区和无障碍语义，不声明 default/base，也不用会删除圆角的 `surface=transparent`。
