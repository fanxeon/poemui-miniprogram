...
- Icon 是展示叶子：不公开 `disabled/click`，交互统一组合 PUI Button + Icon；`name` 只表示 218 个 manifest 稳定名称并映射本地嵌入 WOFF2，`size` 固定 Number+rpx 且限制 8–256。基础 WXML 显式输出 `<pui-icon name="spark" />` 且零 `bind:*`；成功返回 `{ name, source: "font" }`，未知名称按名称去重报告 `unknown-icon` 并显示首字符回退，空 `ariaLabel` 为装饰性 `aria-hidden`。
...
- 2026-07-27 再次访问 TDesign Picker 官方页与官方仓库，并解包固定包 `tdesign-miniprogram@1.15.3` 的 `miniprogram_dist/picker/picker.wxml`、`template.wxml`、`picker.wxss`。运行态发现 PoemUI 仍向当前 Popup 已不存在的 `header` Slot 投递整条 Toolbar，结果只剩无标题、无操作的滚轮；现改为 Popup Header 显示标题、Content 承载滚轮、两列 Footer 承载取消/确认，DateTimePicker 同源继承。
...
- PoemUI 最终公开 25 Props、8 Events、0 Slots、6 Methods。`value` 与 `visible` 分别承担值和显隐受控权，滚动只改变草稿并发布 `pick`，确认固定为 `confirm → change（值变化时）→ visible-change → close`，取消固定为 `cancel → visible-change → close`；`0/false/空字符串` 都按严格身份保留。状态优先级固定为 `error > loading > empty > content`，Retry 不伪造清除 error。
...
- 2026-07-27 再次访问官方 DateTimePicker 页面/仓库并解包固定包 `miniprogram_dist/date-time-picker/date-time-picker.wxml`；固定源码仍只组合 Picker，因此本轮只修 Picker 的 Popup 三分区，DateTimePicker 不新增任何私有 Header、Footer 或 Surface。
- PoemUI 从两个系统 date/time picker 的 27 Props 分裂适配器重建为 PUI Picker 日期时间领域层，最终公开 21 Props、7 Events、0 Slots、6 Methods。年/月/日/时/分/秒动态联动，闰年与月末合法，范围倒置交换，正整数步长保留边界，数字 `0` 为合法受控时间戳；确认固定 `confirm → change（值变化时）→ visible-change → close`，取消回滚草稿。
