...
| 应用级外部授权详情 | “高级版商业授权” Cell 只使用“查阅详情”语义；点击先打开受控 PUI Dialog，确认后调用 `wx.navigateTo` 进入固定 `pages/license/index`，由微信 WebView 加载 `https://poemcoder.com/poem-ui`。Confirm 在导航期间 loading/disabled，失败恢复并保留重试/取消；WebView 加载失败提供复制链接的真实恢复动作。 | H5 不复制“我的”业务页，生产落地页本身是浏览器真相源；Dialog 仍同步组件镜像，微信 WebView 不在 H5 预览伪造。 | 直接在 Cell 点击时无确认离开、用 Toast 冒充详情、接受任意 query URL 形成开放跳转、伪造支付成功，或未配置业务域名就宣称真机可用。 | `MINIPROGRAM_ME_PAGE.md`、`DIALOG.md`、`H5_PREVIEW_COMPATIBILITY.md`；PUI-FB-0457。 |
