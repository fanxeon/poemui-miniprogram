...
| 应用级更新公告 | “更新公告”是我的页真实 Cell，点击打开受控 PUI Popup；Header 展示版本，Content 关闭 Popup 自滚动并由最大 `78vh` 的唯一 PUI ScrollArea 承担滚动，以 Tag + Icon 突出 `component/title/description`，Content 顶部使用受控 TopLoading 表达真实云请求，Footer 使用全宽 PUI Button。只有云端成功进入 success，缓存、本地回退或异常回到 idle。Service 通过独立 `wx.cloud.Cloud` 读取诗上共享环境 `pui_updatelog`，成功缓存，失败回退缓存/包内同形数据并保留来源。公告版本必须与包内 fallback 同形、按版本查重后写入；内容只陈列可由源码、版本差集和验收证明的改动，当前工作树版本不得冒充 npm Registry 已发布。所有 PUI 云集合使用 `pui_` 前缀。 | H5 不复制应用业务页，但必须同步 Popup `contentScrollable` 与 ScrollArea 正 `vh` 高度合同，以及 Popup/Tag/Icon/TopLoading/Button 的组件镜像。多应用共享云服务时，同主体使用环境共享；跨主体使用独立 HTTPS/CloudBase HTTP 服务并按可信 AppID 鉴权。 | 同时启用 Popup Content 和内部 ScrollArea 两个纵向滚动所有者；在页面手写浮层或 Spinner；把缓存/包内回退说成云端成功；未查重就重复插入版本；用无主次字符串、未经验证的“已发布/已上线”或营销文案替代组件改动；客户端携带管理员密钥。 | `MINIPROGRAM_ME_PAGE.md`、`SHARED_MINIPROGRAM_CLOUD_SERVICE.md`、`POPUP.md`、`SCROLL-AREA.md`、`TOP-LOADING.md`；PUI-FB-0421、0440、0447。 |
