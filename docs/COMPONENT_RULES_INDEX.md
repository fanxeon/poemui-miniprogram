...
| `Input` / `Textarea` / `Search` | 原生 focus 在真实 blur、显式 blur、disabled/readonly 或卸载前必须保持；组件同步不得把正在输入的 focus 写回 false。Input 主轨可伸缩，Clear/Loading/suffix/suffix-icon 共用贴右 Trailing；`suffix="slot"` 可组合一个紧凑 PUI IconButton。Input `clearTrigger` 默认 `focus`，显式 `always` 留给 Search 等公开策略；Field 阴影只读取 Provider Token。 | `inputControlSample` 与标准镜像输出同名 Trailing，Clear 默认以 `:focus-within` 显示，Search 显式传递自身策略；重绘后恢复焦点/光标，shadow=off 时 Field 计算阴影为 none。 | 为解决错位给 Search 写绝对定位；在页面写死 Input 阴影或 Clear；新增与 suffix 重复的 right/action Prop；或因 state 同步让输入一聚焦即失焦。 | `INPUT.md`、`TEXTAREA.md`、`SEARCH.md`；PUI-FB-0314、0319、0442、0458。 |
...
| `Steps` connector | default 指示器尺寸/中心、连接间隙、纵向条目间隙固定使用 `44/22/16/12rpx` 语义别名；纵向 x 锚点在中心并做半线宽校正，起止分别距当前/下一指示器外缘一个连接间隙。dot 按自身 `20rpx` 尺寸与 `12rpx` offset 计算。 | 同名别名按 `1px≈2rpx` 镜像为 `22/11/8/6px`；horizontal/vertical 与 default/dot 四种组合使用同一公式。 | 用 `44rpx` 完整尺寸充当中心、只在独立页改 left/top，或用静态字符串测试把错误锚点判为正确。 | `STEPS.md`；PUI-FB-0408、0427。 |
| 一级 Tabbar 目的地与应用标识 | `item.icon/activeIcon` 必须来自当前 Icon manifest，再注册真实页面路由；Codex 目的地使用通用 `code`，其正文仅读取 `pui-codepage` 的 `product=poemui/pageKey=codex/status=published` page/skill 文档，页面不得维护安装或 Skill 文案回退。首页品牌名旁使用 PUI Tag 展示 `poemui-miniprogram/version` 轻量子入口；构建脚本从 `package.json.version` 生成该入口，禁止加载组件总入口或维护第二份版本字符串。第四 Tab 不是用户中心，不注册或渲染头像、昵称、本地资料 Store 或 OpenID；唯一仪表盘 Card 组合透明 BarChart，数据由 metadata、packageComponents 与显式版本差集生成，版本读取公开轻量入口，统计基线固定为 0。“规范”的数量与版本分段并入“基础”但不单列；折叠四类必须优先保留本版发生变化的分类，再按当前数量补足。已有/新增固定使用 Blue/Teal 并显示图例；以透明超小 PUI Button 切换“查看更多 / 收起”时，父级测量真实首尾高度，复用同一 BarChart，以 normal/standard Token 过渡并在完成后才卸载行，低动效为 1ms。Card 不重复无信息增量的标题。服务 CellGroup 保持 `margin-top:auto` 并在短屏正常滚动。低频客服入口使用 Navbar 最左侧 `left` Slot 内的 `text/transparent/small/circle/iconOnly` PUI Button，保留真实 `openType=contact` 和失败反馈，按钮不得显示灰底、边框或外投影。`capsule=true` 下右 Slot 不渲染，`leftBtn` 也不能替代需要平台开放能力的真实 Button。未交付能力明确禁用或提示，不提供假数据、假按钮或空白占位壳。 | Icon 生成链同步同名 glyph；小程序应用级目的地不伪造成 H5 组件预览，只同步它真实消费的组件合同；BarChart 默认配色、显式分段、图例、网格 Token 与零基线必须双端同步，微信客服会话、版本差集选择与页面高度动画不在浏览器伪造。 | 只换 items 字符串而不检查 Icon manifest；在页面硬编码版本、组件分类数量或为版本加载组件总入口；将 draft/archived Skill 或包内内容伪装为云端发布；恢复头像昵称、user-profile 或 OpenID 本地兼容读取；用手写常量、按日期/文件数猜增量或健康度文案冒充组件目录统计；把“规范”混作业务组件分类、在折叠态隐藏本版变化、常驻展示所有低优先级行或保留重复标题；对 `height:auto` 做无效 transition、重建 BarChart 或用固定 max-height 猜内容；给客服按钮增加灰色 Surface、把客服放进 capsule 模式的右 Slot、改成普通 `leftBtn` 点击、原生按钮、满宽主操作或遮挡内容的悬浮入口；保留语义过期空白路由；伪造支付、订单、SKILL 下载、复制或成功状态。 | `MINIPROGRAM_HOME.md`、`MINIPROGRAM_CODEX_PAGE.md`、`MINIPROGRAM_ME_PAGE.md`、`NAVBAR.md`、`TABBAR.md`；PUI-FB-0293、0339、0420、0421、0448、0451、0454、0459、0464。 |
| 应用级更新公告 | “更新公告”是我的页真实 Cell，点击打开受控 PUI Popup；Header 展示版本，Content 关闭 Popup 自滚动并由 `height=auto / maxHeight=60vh` 的唯一 PUI ScrollArea 承担滚动，以 Tag + Icon 突出 `component/title/description`，Surface 顶边使用受控 TopLoading 表达真实云请求，Footer 使用全宽 PUI Button。Popup card 与毛玻璃组件树必须把 ScrollArea 默认遮罩从实底容器色重绑定为同色相半透明材质色；页面不传固定白色补丁，显式 `gradientOverlayColor` 仍可覆盖。只有云端成功进入 success，缓存、本地回退或异常回到 idle。Service 通过独立 `wx.cloud.Cloud` 读取诗上共享环境 `pui_updatelog`，成功缓存，失败回退缓存/包内同形数据并保留来源。公告版本必须与包内 fallback 同形、按版本查重后写入；内容只陈列可由源码、版本差集和验收证明的改动，当前工作树版本不得冒充 npm Registry 已发布。所有 PUI 云集合使用 `pui_` 前缀。 | H5 不复制 Me 业务页，但提供独立 `#/updates` 文档页：Topbar 版本号使用共享 PUI Button 进入，一个连续 Surface 按版本展示组件级改动。站点构建从小程序公告 Service 生成 `preview/release-notes-data.js`，不在浏览器伪造 `wx.cloud` 或实时云查询；小程序 Popup/ScrollArea/Tag/Icon/TopLoading/Button 仍按组件合同同步，frost/Popup card 同样向 ScrollArea 镜像传递材质遮罩 Token。多应用共享云服务时，同主体使用环境共享；跨主体使用独立 HTTPS/CloudBase HTTP 服务并按可信 AppID 鉴权。 | 同时启用 Popup Content 和内部 ScrollArea 两个纵向滚动所有者；在 Me 页面硬编码遮罩色、手写浮层或 Spinner；把缓存/包内回退或 H5 生成数据说成实时云端成功；未查重就重复插入版本；给每条公告再套 Card；用无主次字符串、未经验证的“npm 已发布/已上线”或营销文案替代组件改动；客户端携带管理员密钥。 | `MINIPROGRAM_ME_PAGE.md`、`SHARED_MINIPROGRAM_CLOUD_SERVICE.md`、`H5_PREVIEW_COMPATIBILITY.md`、`RELEASE_0.1.4_H5_CHANGELOG.md`、`POPUP.md`、`SCROLL-AREA.md`、`TOP-LOADING.md`；PUI-FB-0421、0440、0447、0531、0532。 |
| 应用级外部授权详情 | “高级版商业授权” Cell 只使用“查阅详情”语义；点击先打开受控 PUI Dialog，明确提示桌面端浏览体验更佳，并用 `actions` 提供“复制链接 / 直接访问”。复制真实调用 `wx.setClipboardData` 写入固定 `https://poemcoder.com/poem-ui`；直接访问调用 `wx.navigateTo` 进入固定 `pages/license/index`，由微信 WebView 加载同一地址。导航期间两个动作 disabled、直接访问 loading；复制或导航失败保留 Dialog，WebView 加载失败提供返回或复制链接。 | H5 不复制“我的”业务页，生产落地页本身是浏览器真相源；Dialog 仍同步组件镜像，微信 WebView 不在 H5 预览伪造。 | 点击后绕过桌面端提示直接离开、用 Toast 冒充详情或复制成功、接受任意 query URL 形成开放跳转、伪造支付成功，或未配置业务域名就宣称真机可用。 | `MINIPROGRAM_ME_PAGE.md`、`DIALOG.md`、`H5_PREVIEW_COMPATIBILITY.md`；PUI-FB-0457。 |
...
| `AreaChart` / `BarChart` / `Waffle` | 三者是透明只读 `display-leaf`，共享非负有限值、六种主题和零基线语义。AreaChart 小程序用 Canvas 2D、H5 用 SVG，支持 natural/linear/step、叠加/堆叠与强调色到透明填充；BarChart 单序列统一 Blue、多段 Blue/Teal/Violet，并以专用低对比网格显示真实 `0 → max`；Waffle 使用圆润单元渐变、目标 opacity 与 inset outline，超限始终展示有效单位。三者默认 `animated=true/duration=500`，公开 `replay()`，低动效 1ms。 | H5 使用 Area SVG 与 Bar/Waffle DOM，同名 Token/Props/默认色义/缩放公式；标准 PreviewDevice `shadow-safe` 展示正常与边界场景，重播只重置入场状态，不重建业务数据。 | 使用图片、截图或静态写死图形；AreaChart H5 用 Canvas 截图；只靠颜色表达含义；页面穿透设置默认色或伪造坐标；在同一节点组合 `wx:else + wx:for`；给根增加 Card、阴影、毛玻璃、边框或等距 Surface；让动画表示请求成功；让 `showValue=false` 隐藏 Waffle 缩放事实。 | `AREA-CHART.md`、`BAR-CHART.md`、`WAFFLE.md`、`AREA_CHART_GRADIENT_COMPONENT_PLAN.md`；PUI-FB-0460、0461、0462、0463、0465、0466。 |
...
| WXML / WXSS 发布兼容 | 发布 WXML 禁止同节点组合 `wx:else + wx:for`；需要循环的 else 分支使用无实体 `block wx:else` 或完整互斥 `wx:if`。发布组件 WXSS 禁止 `*`、`> *`、`*::before/after`，也禁止导入含 `page`、标签、ID 或属性选择器的全局样式。`common/style/theme.wxss` 只能由消费者 `app.wxss` 经 `theme/utilities.wxss` 或 `theme/theme.wxss` 入口导入；组件从 Provider/全局级联继承 Token。 | H5 使用同名 Token 镜像，不复制小程序 WXSS 导入链；无实体 WXML `block` 不得在 H5 变成额外布局节点，也不能借浏览器选择器创建小程序端不存在的结构或行为。 | 在组件 source 或 `miniprogram_npm` 手工导入/修复全局主题；保留非法条件循环或通配选择器；用实体 view 只为绕过编译；或以手工复制 npm 产物掩盖上传错误。 | `UI_DESIGN_CONTRACT.md`、`POPUP.md`、`BAR-CHART.md`；PUI-FB-0300、0316、0323、0332、0340、0462。 |
...
| 规则 | 适用组件 | 真相源 |
| --- | --- | --- |
| 空 `colorScheme` 只表示继承 Provider，禁止强制浅色 | ActionSheet、PullRefresh、DropdownMenu，以及以后所有使用 Theme Behavior 的组件 | `docs/UI_DESIGN_CONTRACT.md`、对应组件合同、PUI-FB-0524 |
| 透明图表叶子不获得 Surface 效果 | AreaChart、BarChart、Waffle、DonutChart、RadarChart | 各图表合同、`appearance-contract-matrix.js` |
| 连续集合只有集合根可获得 Surface 资格 | CellGroup、List、SortableList 等 | `SORTABLE-LIST.md`、`UI_DESIGN_CONTRACT.md` |
| SortableList 保持连续集合、唯一滚动所有者和父级顺序真相；0.1.4 的 `animated / duration / reduceMotion` 只承诺活动行状态过渡，不把顺序切换描述为兄弟行让位或落位动画。 | SortableList | `SORTABLE-LIST.md`、`RELEASE_0.1.4_H5_CHANGELOG.md`、PUI-FB-0527、PUI-FB-0530 |
| detached 引导只允许一个 Overlay、Panel 与焦点链 | Tour | `TOUR.md`、PUI-FB-0528 |
| H5 浮层阴影必须以实际计算样式证明开关生效 | Popup、Dialog、Sheet、Drawer、Popover、Tour 等 | `RELEASE_0.1.4_H5_CHANGELOG.md`、PUI-FB-0529 |
| 变更必须能按组件反查级别、双端文件、测试和风险 | 0.1.4 及后续版本 | `RELEASE_0.1.4_COMPONENT_CHANGELOG.md` |
