...
2026-07-29 的 0.1.2 发布同步已完成共享云与微信体验版回读。生产环境 `poemcoder-1gkbkid139b08f45` 的 `pui_updatelog` 按稳定 `_id=pui-v0-1-2-20260729` 写入并回读为唯一记录，字段为 `version=v0.1.2 / date=2026-07-29 / status=published`，五组内容与包内 fallback 一致；审计副本见 `.audit/cloud/pui-updatelog-v0.1.2-readback.json`。微信开发者工具完成构建 npm 后，上传界面显示“代码上传成功”，体验版应用版本为 `2.1.0`，其中组件库版本为 `0.1.2`。重新打开 Me 页可见 `74 / 组件`、`562 / 样式`、`8 / 高级`，AreaChart 读屏内容为 `0.1.0 71 → 0.1.2 74`；点击更新公告后 Popup 显示 2026-07-29 与同义五组内容。该运行态视觉证明公告可见，不单独证明本次页面数据一定绕过同形 fallback；生产云服务端的唯一文档回读作为数据写入证据。iOS/Android 真机仍为 `pending-device`。
